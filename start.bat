...
:: Decide which mode to elevate into (default chooser).
set "ELEVATE_ARG=admin"
if /I "%~1"=="gui" set "ELEVATE_ARG=admin-gui"
if /I "%~1"=="cli" set "ELEVATE_ARG=admin-cli"

:: Self-elevate. Using Start-Process with separate -FilePath + -ArgumentList
:: avoids the fragile nested cmd-quoting that breaks on paths with spaces.
:: If UAC is denied or elevation fails, surface the error to the user instead
:: of silently closing the window.
